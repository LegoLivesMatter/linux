...
static int mmp_tdma_config_chan(struct dma_chan *chan, struct dma_slave_config *dmaengine_cfg)
{
	struct mmp_tdma_chan *tdmac = to_mmp_tdma_chan(chan);
	unsigned int tdcr = 0;
...
static int mmp_tdma_probe(struct platform_device *pdev)
{
	...
	if (np) {
		pool = of_gen_pool_get(np, "asram", 0);
		if (!pool) {
	...
	tdev->device.device_config = mmp_tdma_config_chan;

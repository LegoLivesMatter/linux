...
static int pxa1908_apbc_clk_init(struct platform_device *pdev)
{
...
static int pxa1908_apbcp_clk_init(struct platform_device *pdev)
{
	struct pxa1908_clk_unit *pxa_unit;
...
static int pxa1908_mpmu_clk_init(struct platform_device *pdev)
{
	struct pxa1908_clk_unit *pxa_unit;
...
static int pxa1908_apmu_clk_init(struct platform_device *pdev)
{
	struct pxa1908_clk_unit *pxa_unit;
...
static int __init pxa1908_clk_init(void)
{
	int ret;

	ret = platform_driver_register(&pxa1908_apbc_driver);
	if (!ret)
		return ret;
	ret = platform_driver_register(&pxa1908_apbcp_driver);
	if (!ret)
		return ret;
	ret = platform_driver_register(&pxa1908_apmu_driver);
	if (!ret)
		return ret;
	return platform_driver_register(&pxa1908_mpmu_driver);
}
core_initcall(pxa1908_clk_init);

static void __exit pxa1908_clk_exit(void)
{
	platform_driver_unregister(&pxa1908_apbc_driver);
	platform_driver_unregister(&pxa1908_apmu_driver);
	platform_driver_unregister(&pxa1908_apbcp_driver);
	platform_driver_unregister(&pxa1908_mpmu_driver);
}
module_exit(pxa1908_clk_exit);

MODULE_AUTHOR("Duje Mihanović <duje.mihanovic@skole.hr>");
MODULE_DESCRIPTION("Marvell PXA1908 Clock Driver");
MODULE_LICENSE("GPL");

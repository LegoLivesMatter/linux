...
static const struct snd_soc_dapm_widget pm860_widgets[] = {
	SND_SOC_DAPM_AIF_IN("LIN", NULL, 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("RIN", NULL, 0, SND_SOC_NOPM, 0, 0),

	SND_SOC_DAPM_OUTPUT("OUT"),
};

static const struct snd_soc_dapm_route pm860_routes[] = {
	{"LIN", NULL, "Voice Playback"},
	{"RIN", NULL, "Voice Playback"},
...
static unsigned int pm860_read(struct snd_soc_component *codec,
				unsigned int reg)
{
	struct pm860_private *pm860_priv = snd_soc_component_get_drvdata(codec);
...
static int pm860_write(struct snd_soc_component *codec, unsigned int reg,
	unsigned int value)
{
	struct pm860_private *pm860_priv = snd_soc_component_get_drvdata(codec);
	struct pm80x_chip *chip = pm860_priv->chip;
...
int pm860_hw_init(struct snd_soc_component *codec)
{
	return 0;
}
EXPORT_SYMBOL_GPL(pm860_hw_init);

static int pm860_set_bias_level(struct snd_soc_component *codec,
	...
	struct snd_soc_dai *dai)
{
	...
	if (atomic_read(&tdm_count) == 0) {
		...
		if (!snd_soc_component_read(codec, PM860_MAIN_POWER_REG)) {
			usleep_range(1000, 1200);
			snd_soc_component_write(codec, PM860_MAIN_POWER_REG, 0x1);
			while (count--) {
				tmp = snd_soc_component_read(codec,
		...
		tmp = snd_soc_component_read(codec, PM860_TDM_SETTING1);
		tmp |= (1 << 7);
		snd_soc_component_write(codec, PM860_TDM_SETTING1, tmp);
#endif
		/* enable pll_compat_mode */
		tmp = snd_soc_component_read(codec, PM860_TDM_PLL_DIV);
		tmp |= (1 << 5);
		snd_soc_component_write(codec, PM860_TDM_PLL_DIV, tmp);
		/* align the tdm slot space with MAP */
		snd_soc_component_write(codec, PM860_TDM_SETTING15, 0x10);
		/* align fsyn pulse width with MAP */
		snd_soc_component_write(codec, PM860_TDM_SETTING5, 0x02);
		snd_soc_component_write(codec, PM860_TDM_SETTING6, 0xa0);

		/* enable the serial audio interface */
		tmp = snd_soc_component_read(codec, PM860_DIG_BLOCK_EN_REG2);
		tmp |= 1;
		snd_soc_component_write(codec, PM860_DIG_BLOCK_EN_REG2, tmp);
		/* apply tdm config */
		tmp = (APPLY_TDM_CONF | APPLY_CKG_CONF);
		snd_soc_component_write(codec, PM860_TDM_APPLY_CONF, tmp);
	}
	...
	struct snd_soc_dai *dai)
{
	...
	if (atomic_read(&tdm_count) == 0)
		snd_soc_component_write(codec, PM860_MAIN_POWER_REG, 0x0);
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	tdm_ctrl_reg6 = snd_soc_component_read(codec, PM860_TDM_SETTING16);
	tdm_ctrl_reg7 = snd_soc_component_read(codec, PM860_TDM_SETTING17_1);
	/* bit size */
	...
	snd_soc_component_write(codec, PM860_TDM_SETTING16, tdm_ctrl_reg6);
	snd_soc_component_write(codec, PM860_TDM_SETTING17_1, tdm_ctrl_reg7);

	/* apply tdm config */
	tdm_ctrl_reg1 = (APPLY_TDM_CONF | APPLY_CKG_CONF);
	snd_soc_component_write(codec, PM860_TDM_APPLY_CONF, tdm_ctrl_reg1);
		...
		unsigned int fmt)
{
	...
	tdm_ctrl1 = snd_soc_component_read(codec, addr);
	...
	ret = snd_soc_component_write(codec, addr, tdm_ctrl1);
	if (ret < 0)
		return ret;

	/* apply tdm config */
	tdm_ctrl1 = (APPLY_TDM_CONF | APPLY_CKG_CONF);
	snd_soc_component_write(codec, PM860_TDM_APPLY_CONF, tdm_ctrl1);
...
static void tdm_mic_channel_update(struct snd_soc_component *codec, int channel,
	int stream, int slot)
{
	...
	if (slot == 1) {
		val = snd_soc_component_read(codec, reg);
		val &= ~0xf;
		val |= stream;
				...
				unsigned int rx_num, unsigned int *rx_slot)
{
	...
	snd_soc_component_write(codec, PM860_TDM_APPLY_CONF, tdm_apply_ctrl);
				...
				unsigned int rx_num, unsigned int *rx_slot)
{
	...
	rx_slot_1 = snd_soc_component_read(codec, PM860_TDM_SETTING14);

	/* FixME: For out1, we assume it use the [4-7] bits */
	rx_slot_1 &= ~0xff;
	rx_slot_1 |= rx_slot[0];
	rx_slot_1 |= (rx_slot[1] << 4);
	snd_soc_component_write(codec, PM860_TDM_SETTING14, rx_slot_1);
				...
				unsigned int rx_num, unsigned int *rx_slot)
{
	...
	rx_slot_1 = snd_soc_component_read(codec, PM860_TDM_SETTING13);

	rx_slot_1 &= ~0xff;
	rx_slot_1 |= rx_slot[0];
	rx_slot_1 |= rx_slot[1] << 4;

	snd_soc_component_write(codec, PM860_TDM_SETTING13, rx_slot_1);
...
static void pm860_remove(struct snd_soc_component *codec)
{
	pm860_set_bias_level(codec, SND_SOC_BIAS_OFF);
}

#ifdef CONFIG_PM
static int pm860_suspend(struct snd_soc_component *codec)
{
...
static int pm860_resume(struct snd_soc_component *codec)
{
...
static int pm860_probe(struct snd_soc_component *codec)
{
	struct pm860_private *pm860_priv = snd_soc_component_get_drvdata(codec);
	struct pm80x_chip *chip = pm860_priv->chip;
	int ret = 0;
...
struct snd_soc_component_driver soc_codec_dev_pm860 = {
	.probe   = pm860_probe,
	.remove  = pm860_remove,
	...
	.dapm_widgets = pm860_widgets,
	.num_dapm_widgets = ARRAY_SIZE(pm860_widgets),
	.dapm_routes = pm860_routes,
	.num_dapm_routes = ARRAY_SIZE(pm860_routes),
};

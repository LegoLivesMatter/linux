...
static unsigned int map_read(struct snd_soc_component *codec,
				unsigned int reg)
{
	struct map_fe_dai_private *map_fe_dai_priv;
	struct map_private *map_priv;

	map_fe_dai_priv = snd_soc_component_get_drvdata(codec);
	map_priv = map_fe_dai_priv->map_priv;

	return map_raw_read(map_priv, reg);
}

static int map_write(struct snd_soc_component *codec, unsigned int reg,
	unsigned int value)
{
	struct map_fe_dai_private *map_fe_dai_priv;
	struct map_private *map_priv;
...
static const struct snd_kcontrol_new dac1_in_mux =
	SOC_DAPM_ENUM("DAC1 in mux", dac1_in_mux_enum);
...
static const struct snd_kcontrol_new dac2_in_mux =
	SOC_DAPM_ENUM("DAC2 in mux", dac2_in_mux_enum);
...
static const struct snd_kcontrol_new i2s3_func_sel_mux =
	SOC_DAPM_ENUM("i2s3_func_sel", i2s3_func_sel_mux_enum);

static const struct snd_soc_dapm_widget map_dapm_widgets[] = {
	SND_SOC_DAPM_MUX("i2s3_func_sel", SND_SOC_NOPM, 0, 0, &i2s3_func_sel_mux),

	SND_SOC_DAPM_SWITCH("i2s3_bt_vc", SND_SOC_NOPM, 0, 0,
				&i2s3_bt_vc_control),

	SND_SOC_DAPM_ADC("AOUT3 record", NULL, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_SWITCH("out1_hs_en", SND_SOC_NOPM, 0, 0,
				&out1_hs_en_control),

	SND_SOC_DAPM_SWITCH("out1_spkr_en", SND_SOC_NOPM, 0, 0,
	...
	SND_SOC_DAPM_MUX("DAC1 in mux", SND_SOC_NOPM, 0, 0, &dac1_in_mux),
	...
	SND_SOC_DAPM_MUX("DAC2 in mux", SND_SOC_NOPM, 0, 0, &dac2_in_mux),
...
static int map_add_widgets(struct snd_soc_component *codec)
{
...
static int map_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	return 0;
}
...
static int map_probe(struct snd_soc_component *codec)
{
	struct map_fe_dai_private *map_fe_dai_priv;

	map_fe_dai_priv = snd_soc_component_get_drvdata(codec);
	map_fe_dai_priv->codec = codec;
	codec->regmap = map_fe_dai_priv->regmap;
...
static void map_remove(struct snd_soc_component *codec)
{
	return;
}

struct snd_soc_component_driver soc_codec_dev_map = {
	.probe   = map_probe,
	.remove  = map_remove,
	.read = map_read,
	.write = map_write,

...
static int mmp_pcm_hw_params(struct snd_soc_component *component, struct snd_pcm_substream *substream,
...
static int mmp_pcm_open(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	...
	pcm_hardware =
		(struct pcm_hardware_info *)component->dev->platform_data;
...
static int mmp_pcm_mmap(struct snd_soc_component *component, struct snd_pcm_substream *substream,
...
static void mmp_pcm_free_dma_buffers(struct snd_soc_component *component, struct snd_pcm *pcm)
{
	...
	if (!gpool) {
		gpool = of_gen_pool_get(np, "asram", 0);
		if (!gpool)
			return;
	}
...
static int mmp_pcm_preallocate_dma_buffer(struct snd_soc_component *component, struct snd_pcm_substream *substream,
								int stream)
{
...
static int mmp_pcm_new(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd)
{
	struct snd_pcm_substream *substream;
...
static int mmp_pcm_close(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	return snd_dmaengine_pcm_close_release_chan(substream);
}

static int mmp_pcm_ioctl(struct snd_soc_component *component, struct snd_pcm_substream *substream, unsigned int cmd,  void *arg)
{
	return snd_pcm_lib_ioctl(substream, cmd, arg);
}

static int mmp_pcm_trigger(struct snd_soc_component *component, struct snd_pcm_substream *substream, int cmd)
{
	return snd_dmaengine_pcm_trigger(substream, cmd);
}

static snd_pcm_uframes_t mmp_pcm_pointer(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	return snd_dmaengine_pcm_pointer(substream);
}

static struct snd_soc_component_driver mmp_soc_platform = {
	.open		= mmp_pcm_open,
	.close		= mmp_pcm_close,
	.ioctl		= mmp_pcm_ioctl,
...
int mmp_pcm_platform_register(struct device *dev)
{
	...
	return snd_soc_register_component(dev, &mmp_soc_platform, NULL, 0);
}
